...
class MultiFrequencyMonitor:
    ...
    def setup_monitor_groups(self):
        """初始化监控分组"""
        if len(BATCHES) >= 1: self.monitor_groups["high_frequency"] = BATCHES[0]
        if len(BATCHES) >= 2: self.monitor_groups["medium_frequency"] = BATCHES[1]
        if len(BATCHES) >= 3:
            all_symbols = []
            for batch in BATCHES[2:]:
                all_symbols.extend(batch)
            self.monitor_groups["low_frequency"] = all_symbols
        
        # 将手动仓位加入高频监控
        positions = strategy_state.get("positions", {})
        for symbol, position in positions.items():
            if position.get("manual", False) and symbol not in self.monitor_groups["high_frequency"]:
                self.monitor_groups["high_frequency"].append(symbol)
                
        logging.info(f"监控分组就绪: 高频{len(self.monitor_groups['high_frequency'])} 中频{len(self.monitor_groups['medium_frequency'])} 低频{len(self.monitor_groups['low_frequency'])}")
